...
实际上，git会将远程的分支，在用户的本地创建一个不能进行本地修改的 本地分支，我们称之为 远程本地追踪分支 。远程本地追踪分支 的名称为 <remote>/<branch>。其中remote一般为origin。

remote实际上代表的就是一个远程仓库地址的别名。
...
可以通过下面的命令，可以将remote仓库上的所有分支中的内容，下载到本地，并且写入到远程本地追踪分支中。
...
补充提交文件，为了避免commit log出现很多没有意义的提交历史

git commit --amend


列出全部的本地分支以及远程仓库本地追踪分支
...
//删除文件。因为git只追踪工作区中的修改，所以git add能够追踪所有的工作区修改。git rm命令等价于
...
//移动文件。因为git只追踪工作区中的修改，所以git add能够追踪所有的工作区修改。git mv命令等价于
...
然后在进行merge的分支上面进行应用。rebase命令区别于merge的主要差别
(1) 在提交日志上不会形成一个分叉。
(2) 假设主干还有分支的开发人员是两拨人，任务是相互独立的。rebase的合并操作是由分支的开发人员发起的，所以主干的开发人员可以不需要感知到需要进行merge这件事情，fast-forwad到应用分支上提交的rebase的提交即可

svn上的分支合并应该就是属于rebase类型的合并(这可能也是为何我一直觉得git的commit log很奇怪的原因了)
对于git来讲，一个良好的实践，是对于本地的分支的开发合并，使用rebase来简化提交历史。否则在远程仓库中就会出现本地分支的merge操作。而这些本地分支的merge操作通常没有什么意义
而对于需要打算推送到远程仓库的分支的合并，则只能使用merge。因为rebase一个会被push的远程仓库的分支会带来灾难性的后果，不能work
...
git checkout master
git merge target_branch                       //master 采用fastward的方式合并到target_branch
...
在github，gitlab中创建自己的项目，设置对应的权限
