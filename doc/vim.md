...
gI 回到上一个窗口上一次进入insert mode的位置，并且进入insert mode
...
### 快速拷贝mapping
编写代码时，我们需要大量进行拷贝黏贴。通常我们需要在当前编辑位置，到其他编辑位置拷贝内容，然会回到编辑位置
然后再跑到另外的位置拷贝代码，如此重复。所以每次在选定拷贝内容之后，都要yank，然后回到之前的编辑位置，然后再p
有时还要回到之前的位置继续选择拷贝的内容，需要比较多的keystroke
...
signature help, 只要在插入模式下，将光标通过删除键移动函数括号中,号的后面，就会浮出来，并且在退出插入模式之前不会消失
