...
// 下面的讨论已经过时。目前最为高效的开发方式，是编辑虚拟机直接登录外网测试开发机，然后将文件同步到测试开发机上。这样子就可以解决测试开发机与外部环境不通的问题。
...
(1) 为了能够满足首先跳转到若干个 intermedia_host，然后再通过 intermedia_host 跳转到真正目标的 target_host，由于 intermedia_host不能保存密钥信息，也不希望整条登录链路上的机器都要进行配置，
    所以采用了将密钥请求由原路中转到 远端的技术方案，并且agent forwarding是从远端进行配置的
(2) 这些中间的 intermedia_host 有自己的区别于 target_host 的认证方式。比如上面我们首先要登录一个中间的虚拟机。在整个登录链路上，拥有自己认证方式的intermedia_host有任意个。
...
(3) 为了让用户少输入信息达到 无密钥登录的功能，ssh终端在本地查找密钥时，会将所有可能的密钥都尝试一遍。
