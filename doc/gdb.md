...
  获取文件编译位置：  objdump -g test|grep DW_AT_name|grep test.cc

  获取编译目录位置： objdump -g test|grep DW_AT_comp_dir
...
这是后gdb就知道从 DEST_DIR/目录下，进一步搜索 源代码的文件编译位置 /project/...
...
gdb target_file  //target file是待调试进程的二进制可执行文件，里面包括了二进制信息
...
通过cat /proc/sys/kernel/core_pattern 就能知道coredump生成的位置
...
输入 gdb target coredump_file后，就会启用gdb进行调试。此时gdb dashboard中的东西已经不能打印出来了，需要手动输入gdb的命令来进行分析
...
info registers  //就能获取全部寄存器的值。其中在x86体系架构下，rip就是指令寄存器。

x/20i $pc  显式 $pc指针后20行的汇编代码

x/20i $pc - 40  显式 $pc 指针前40个地址的汇编代码地址，显式20条。用于展现崩溃出现附近的汇编代码
...
  (1) 获得这类复杂结构的符号定义，并且使用内存结构去迭代访问

  (2) 获得这类复杂结构的符号定义，并且使用符号上面的函数去对确定内存的状态
