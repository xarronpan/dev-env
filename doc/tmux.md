...
tmux-yand的主要功能是在屏幕上进行copy选定之后，按Y即可将copy的内容paste到当前命令行中。可以当做是命令行补全来进行使用。
...
tmux-finger是 copycat+tmuxyank 的加强版。只需要一个 prefix + a  (进行了remap。原来的功能是find-window，几乎很少用到)，就会进入finger模式，并能够通过字母按键将 url filepath ip numbers 等命令行参数中可能会用到的东西paste到命令行中
...
在希望从终端中copy参数到命令行时，首先应该使用finger，然后是tmux-jumper， 最后再考虑使用copycat+tmux-yank。而copy cat也仅仅需要考虑使用 / 的正则的方式来进行查找。这是因为对于git status的场景下，zsh与fpp都是更加适合的工具。
...
在经过tmux-finger, tmux-jump, copy-cat, tmux-yank的配置之后，其实只有需要拷贝到windows下面进行处理的东西，才需要使用鼠标来进行拷贝 (鼠标进行拷贝的速度其实是较慢的)

另外一种情况则是小段内容的跨pannel拷贝 (实际上是绝大多数的情况)。这是因为跨pannel的拷贝需要移动光标，并且做一堆按键操作，还真不一定有shift+鼠标的方式来得快

只有带拷贝的内容较为复杂，跨行、跨page的情况下，才考虑使用tmux，tmux与vi之间的缓冲区来进行拷贝，能够节省一些时间

在有些情况下鼠标的操作就是要快于键盘的操作，我们一定不要迷信什么情况下使用键盘就是要快于使用鼠标的说法，一定要实事求是
...
注意目前每次上报gist的时候，都需要刷新分配个pet的gist token，pet sync的时候token才能正常工作
