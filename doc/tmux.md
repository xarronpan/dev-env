...
另外一个选择复制内容的技巧。使用该插件将光标定位到需要复制内容的开始,输入v，开始复制 。然后观察需要拷贝内容的结束的字母，比如说'a'。
输入fa，这样子tmux的行为就会像vim一样，选择光标所在位置到光标开始第一个'a'所在位置之间的内容。如果这个位置不是目标位置，则输入;键往后搜索，输入,键往前搜索
这样子就可以很快地定位到要选择的内容了
